(() => {
  ...
  // src/index.ts
  var parse = (str, args, i = 0) => str.replace(/%s/g, () => args[i++]);
  var messagePattern = /^(Error: )?Minified React error/i;
  var urlPattern = /https?:\/\/[^\s]+/;
  var invariantPattern = /https?:\/\/react\.dev\/errors\/(\d+)/;
  function decodeDetails(message) {
    const fallback = {
      message,
      invariant: void 0,
      url: void 0
    ...
    const invariant = searchParams.get("invariant") || url.match(invariantPattern)?.at?.(1);
    if (!invariant) return fallback;
    if (!Object.hasOwn(collection_default, invariant)) return fallback;
    const template = collection_default[invariant];
    return {
  ...
  var decode = (message) => decodeDetails(message).message;
  decode.details = decodeDetails;
  decode.collection = collection_default;
  ...
  document.getElementById("error").addEventListener("input", (event) => print(event.target.value));
